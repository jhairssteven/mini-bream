...
from backseat.msg import telem, locg
...
class RvizPath:
    ...
    def __init__(self, init_lat=rospy.get_param("goal_lat"), init_lon=rospy.get_param("goal_lon")):
        #rospy.init_node('state_pub')
        self.ref_trajectory_pub = rospy.Publisher('/ref_path', Path, queue_size=1)
        self.waypts_pub = rospy.Publisher('/mission_waypts', Marker, queue_size=1)
    ...
    def pub_waypt_markers(self, utm_waypts):
        ...
        while not rospy.is_shutdown():
            ...
            #rospy.loginfo("Published mission waypoints")
            #rate.sleep()
    ...
    def pub_ref_trajectory(self, utm_waypts):
        ...
        while not rospy.is_shutdown():
            ...
            #rospy.loginfo("Published reference path with {} waypts".format(len(utm_waypts)))
            rate.sleep()    
...
class RvizPos:
    ...
    def __init__(self, init_lat=rospy.get_param("goal_lat"), init_lon=rospy.get_param("goal_lon")):
        ...
        self.i = 0
        rospy.init_node('rviz_pos')

        # Subscribers

        if rospy.get_param("sim_enable"):
            rospy.Subscriber('/wamv/sensors/gps/gps/fix', NavSatFix, self.gps_callback, queue_size=1)
        else:
            rospy.Subscriber('/ublox_gps/fix', NavSatFix, self.gps_callback, queue_size=1)
            
        rospy.Subscriber('/compass_bearing_deg', Float32, self.heading_callback) # TODO: change to the one published by VRX_topic_adapter
        rospy.Subscriber('/telemetry', telem, self.telem_callback)
        rospy.Subscriber('/ref_path', Path, self.ref_path_callback)
        rospy.Subscriber('/way_gps', locg, self.way_gps_callback) # TODO: change to the value published on telemetry or stat
        
        # Marker publishers
        self.ilosHeadingMarkerPub = rospy.Publisher("/heading/marker/ilos", Marker, queue_size=2)
        self.pidrOutputMarkerPub = rospy.Publisher("/heading/marker/pidr_output", Marker, queue_size=2)
        self.currPoseMarkerPub = rospy.Publisher("/curr_marker", Marker, queue_size=2)
        self.goalPoseMarkerPub = rospy.Publisher("/goal_marker", Marker, queue_size=2)
        self.traversedPathPub = rospy.Publisher("/traversed_path", Path, queue_size=2)
        
        # Number-only publishers
        self.ilosHeadingPub = rospy.Publisher("/heading/value/ilos", Float32, queue_size=1)
        self.currAcuteHeadingPub = rospy.Publisher("/heading/value/curr_acute_heading", Float32, queue_size=1)
        #self.PIDrInputPub = rospy.Publisher("/pid/rotational/input", Float32, queue_size=1)
        self.PIDrErrorPub = rospy.Publisher("/pid/rotational/error", Float32, queue_size=1)
        #self.PIDrSystemResponsePub = rospy.Publisher("/pid/rotational/system_response", Float32, queue_size=1)
    ...
    def current_pose_pub(self):
        if (self.curr_lat is not None):        
            ...
            curr_x = 0
            curr_y = 0
        #rospy.loginfo("(x,y, yaw (rads)): %s, %s, %s", curr_x, curr_y, curr_heading)
        ...
        #rospy.loginfo("curr (x, y): %s, %s", round(curr_x, 3), round(curr_y, 3))
        ...
        rst_trav_path = rospy.get_param("rst_trav_path") if rospy.has_param("rst_trav_path") else False
        if rst_trav_path:
            self.ref_path.poses = self.ref_path.poses[-1:]
            rospy.set_param("rst_trav_path", False)
        else:
    ...
    def telem_callback(self, data):
        if self.init_UTM_x == None and self.init_UTM_y == None:
            #rospy.logerror("Waiting for /ref_path to set origin frame coordinates")
            rospy.logerr("init_UTM_x and init_UTM_y have not yet been initialized. Waiting for /ref_path")
            return
