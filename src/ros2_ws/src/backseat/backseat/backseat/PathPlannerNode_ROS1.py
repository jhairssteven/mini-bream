...
from backseat_msgs.msg import Locg, DoMissionAction, DoMissionResult, DoMissionFeedback
from visualization_msgs.msg import Marker
from geometry_msgs.msg import Vector3Stamped, Twist, Pose, Twist, PoseStamped
...
class PathPlannerNode:
    ...
    def __init__(self, node_name='PathPlanner'):
        ...
        self.angle_idx = 0
        self.max_speed = rclpy.get_param("max_speed")
        self.min_speed = rclpy.get_param("min_speed")
        ...
        self.sim_enable = rclpy.get_param("sim_enable")
    ...
    def init_app(self):
        ...
        rclpy.init_node(self.node_name, anonymous=False)
        self.rate = rclpy.Rate(10)
        # Subscribers
        if self.sim_enable:
            rclpy.Subscriber('/wamv/sensors/gps/gps/fix', NavSatFix, self.__read_position_cbk, queue_size=1)
            rclpy.Subscriber('/wamv/sensors/gps/gps/fix_velocity', Vector3Stamped, self.__read_speed_cbk, queue_size=1)
            rclpy.Subscriber('/wamv/sensors/imu/imu/data', Imu, self.__read_imu_cbk, queue_size=1)
            rclpy.Subscriber("/vrx/debug/wind/direction",Float64, self.__get_wind_dir_cbk)
            rclpy.Subscriber("/vrx/debug/wind/speed", Float64, self.__get_wind_speed_cbk)
        else:
            rclpy.Subscriber('/ublox_gps/fix', NavSatFix,self.__read_position_cbk, queue_size=1)
            rclpy.Subscriber('/compass_bearing_deg', Float32, self.__read_imu_cbk, queue_size=1)

        # Publishers
        self.left_pub = rclpy.Publisher('/wamv/thrusters/left_thrust_cmd', Float32, queue_size=10)
        self.right_pub = rclpy.Publisher('/wamv/thrusters/right_thrust_cmd', Float32, queue_size=10)
        # Debug publishers
        self.working_waypoint_pub = rclpy.Publisher("/goal_marker", Marker, queue_size=2)
        self.xtrac_err_pub = rclpy.Publisher('/xtrac_err_abs', Float32, queue_size=1)
        self.head_err_pub = rclpy.Publisher('/head_err_deg', Float32, queue_size=1)
        self.lookahead_pub = rclpy.Publisher('/lookahead', Float32, queue_size=1)
        self.way_gps_pub = rclpy.Publisher('/way_gps', Locg, queue_size=1)
        
        # Visuals
        self.wk_path_pub = rclpy.Publisher("/working_path",Path,queue_size=1)
        self.orig_path_pub = rclpy.Publisher("/original_path",Path,queue_size=1)
        ...
        self._as = actionlib.SimpleActionServer(self._as_name, 
                                                ...
                                                auto_start = False)
        self._as.start()
        rclpy.loginfo(f'Server initialized. Waiting for new mission...')
    ...
    def __read_speed_cbk(self,data):
        ...
        #rclpy.loginfo('current_speed:{}'.format(self.current_speed))
    ...
    def __publish_veh_output(self,data):
        ...
        KP = rclpy.get_param("kp") if rclpy.has_param("kp") else 3.6
        KI = rclpy.get_param("ki") if rclpy.has_param("ki") else 2.553
        KD = rclpy.get_param("kd") if rclpy.has_param("kd") else 3.381
        self.head_u = KP*head_err + KI*self.head_i + KD*derivative
        self.head_prev_error = head_err
        up = rclpy.get_param("max_speed")
        low = rclpy.get_param("min_speed")
        x_up = 0.15
        x_low = 0.4
        m = (up-low)/(x_up-x_low)
        b = up - m*x_up
        ...
        speed = rclpy.get_param("fw_speed") if rclpy.has_param("fw_speed") else c_speed
        # ==========================
    ...
    def __update_follower(self):
        ...
        x_ref,y_ref,_,_ = utm.from_latlon(rclpy.get_param('goal_lat'),rclpy.get_param('goal_lon'))
    ...
    def __run(self, goal):
        ...
        rclpy.loginfo('')
        rclpy.loginfo('-'*20)
        rclpy.loginfo(f'[{goal.id}] Executing new mission...')
        rclpy.loginfo("goal.action received:")
        rclpy.loginfo(goal)
        ...
        if self.mission_complete:
            ...
            rclpy.loginfo('Mission completed')
            rclpy.loginfo('-'*20)
            rclpy.loginfo('')
    ...
    def __main(self):
        ...
        rclpy.loginfo('Creating Mission')
        self.mission = NavigationTools.Mission(filename=rclpy.get_param("mission"))
        self.path_follower = PathFollower(mission=self.mission, path_creator=DubinsPath)
        rate = rclpy.Rate(10)
        while not rclpy.is_shutdown():
            try:
                self.__update_follower()
                rate.sleep()
            except rclpy.ROSInterruptException as ROSe:
                rclpy.logwarn(ROSe)
                break
