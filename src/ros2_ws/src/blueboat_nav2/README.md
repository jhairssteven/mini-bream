...
/rslidar_points
  ...
  → SmacPlannerHybrid (Dubins, footprint 1.20×0.93 m)
...
|----------|----------------|-----------------|------|
| Simulation | `blueboat_sim` | `blueboat/base_link` | Gazebo GT odom |
| Real boat | `blueboat` | `blueboat/blueboat/base_link` | `/odom` (`gps_map_odom`) |
...
Footprint is the Blue Robotics deployed hull (1.20 m × 0.93 m). `inflation_radius` is
≥ the circumscribed radius (~0.76 m) so soft costs cover the full footprint during Smac
search; hard collisions still use the polygon. Adjust `minimum_turning_radius` for your
operating speed.
...
Airy point cloud header stamp is lagging odometry/TF by tens of seconds. Nav2 needs
`/scan` header time aligned with `/odom` / TF. The launch file runs
