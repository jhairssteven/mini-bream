...
def _progress_line(phase: str, elapsed_s: float, total_s: float) -> str:
    ...
    return f"[ilos] {phase:<12} {elapsed_s:5.1f}/{total_s:.0f}s  ({pct:3.0f}%)  ~{left:.0f}s left"
...
def run_boat_trial(
    ...
    if need_gps:
        ...
        if fix is None:
            print("[ilos] wait_gps failed", flush=True)
            return float("inf")
    ...
    try:
        ...
        if not wait_for_xte(wait_xte_timeout_s, cfg):
            print("[ilos] wait_xte timed out — scoring from log if available", flush=True)
