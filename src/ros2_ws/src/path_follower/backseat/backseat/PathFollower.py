...
class ReedsSheppPath:
    def __init__(self, node, mission, turn_radius=None,
                 step_size=None, log=True,):
        self.turning_radius = node.get_param("dubins_radius")   # Meters
        self.step_size = node.get_param("dubins_step_size")     # Meters
        self.node.get_logger().debug(f"Reeds-Shepp    radius {self.turning_radius}, step {self.step_size}")
...
class PathFollower:
    ...
    def  update(self, current_wp, speed):
        ...
        if(self.current_wp.pose.gps_lat != 0):
            ...
            #node.loginfo(f"orig_index, work_index: {self.orig_index}, {self.work_index}")
            ...
            #node.logdebug('ye {} idx: {}, len_path: {}, coord: {},{}, tgt: {},{}'.format(self.ye,self.work_index,len(self.working_path),utm_x,utm_y,tgt_x,tgt_y))
            # ======================================================================
            ...
            if self.work_index >= (len(self.working_path)-self.HANDOVER_OFFS):
                ...
                if self.lap_ctr >= self.NO_OF_LAPS:
                    ...
                    self.node.get_logger().debug('Mission Completed')
                else:
                    self.orig_index = 1
                    self.work_index = 1
                    self.ye = 0
